...
#define RF_CSD_PORT					PORT_A								//Shutdown control, should be High for RX or TX
#define RF_CSD_BIT					GPIO_11
...
#define I2C1_SCL_BIT				GPIO_6

#define I2C1_SDA_PORT				PORT_B
#define I2C1_SDA_BIT				GPIO_7
...
typedef enum
{
	HWE_NO_EVENT			= 0b000000000000000000000,					//initial state - no events
	HWE_UART1_RX_EVENT		= 0b000000000000000000001,					//a new character was received by UART1 (user input)
	HWE_UART2_RX_EVENT		= 0b000000000000000000010,					//a new character was received by UART2 (camera)
	HWE_LPUART_RX_EVENT		= 0b000000000000000000100,					//a new character was received by LPUART (GPS)
	HWE_UART1_COMMAND		= 0b000000000000000001000,					//characters received by UART1 made up a full command
	HWE_LPUART_COMMAND		= 0b000000000000000010000,					//characters received by LPUART made up a full command
	HWE_BUTTON_PRESS		= 0b000000000000000100000,					//button was pressed
	HWE_BUTTON_RELEASE		= 0b000000000000001000000,					//button was released
	HWE_ALARM_A				= 0b000000000000010000000,					//Alarm A signaled by RTC
	HWE_LPTIMER				= 0b000000000000100000000,					//generic timer event
	HWE_INPUT_TIMEOUT		= 0b000000000001000000000,					//user input inactivity timer
	HWE_RADIO_TIMEOUT		= 0b000000000010000000000,					//Radio task timer
	HWE_GPS_TIMEOUT			= 0b000000000100000000000,					//GPS task timer
	HWE_CAMERA_TIMEOUT		= 0b000000001000000000000,					//Camera task timer
	HWE_DIO0				= 0b000000010000000000000,					//interrupt on DIO0 line, can be RX, TX or CAD done event
	HWE_DIO3				= 0b000000100000000000000,					//interrupt on DIO3 line, can be reception of a valid header or PayloadCrcError
//artificial events indicating activities in progress, meant to prevent MCU going to low-power mode to allow the activities to finish
	HWE_UART1_TRANSMITTING	= 0b000001000000000000000,					//user message transmission is in progress, reset by ISR
	HWE_UART2_TRANSMITTING	= 0b000010000000000000000,					//transmission to camera is in progress, reset by ISR
	HWE_LPUART_TRANSMITTING	= 0b000100000000000000000,					//transmission to GPS is in progress, reset by ISR
	HWE_RADIO_RECEIVING		= 0b001000000000000000000,					//radio is receiving
	HWE_RADIO_TRANSMITTING	= 0b010000000000000000000,					//radio is transmitting
	HWE_KEEP_CPU_RUNNING	= 0b100000000000000000000,					//prevent deep sleep
} Hardware_Events;
...
int32_t uart1Receive(uint8_t *data, uint32_t length);
...
void addToUart1MsgAsHex(uint8_t byte);
