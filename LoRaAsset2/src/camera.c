...
//	static uint16_t dataSize;
	...
	case PSMC_GET_DATA: {
		...
		startLpUartMsg(RC_PICTURE_DATA);											//move data to computer

		for(i = chksum = 0; i < dataSize + 4; i++) {								//calculate image packet check sum
			addToLpUartMsg(Uart2RxBuffer[i]);
		...
		if(chksum != Uart2RxBuffer[i])
			...
			addToLpUartMsg(Uart2RxBuffer[i++]);										//copy image packet
			addToLpUartMsg(Uart2RxBuffer[i]);
			sendLpUartMessage();
		}
	...
	case CTC_GET_NEXT_DATA:
		if(CTS_GETTING_PICTURE == state) {
			if(++packetsTracker.packetNumber >= packetsTracker.packetsNumber) {
				...
				startLpUartMsg(RC_END_PICTURE);										//signal transfer completion to external consumer
				sendLpUartMessage();
